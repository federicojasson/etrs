...
/**
 * Represents the /treatment/search service.
 */
class Search extends \App\Service\External {
	...
	protected function execute() {
		...
		if (! is_null($expression)) {
			...
			$queryBuilder
				->andWhere('MATCH(t.name) AGAINST(:expression BOOLEAN) > 0')
				->setParameter('expression', $expression);
		}
		
		// Sets the sorting criteria
		foreach ($sortingCriteria as $sortingCriterion) {
			$queryBuilder->addOrderBy('t.' . $sortingCriterion['field'], $sortingCriterion['direction']);
		}
		...
		// Sets an output
		$this->setOutputValue('results', $treatments, createArrayFilter(function($treatment) {
