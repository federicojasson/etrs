...
/**
 * Represents the /diagnosis/edit service.
 */
class Edit extends \App\Service\External {
	...
	protected function execute() {
		...
		// Gets the diagnosis
		$diagnosis = $app->data->getRepository('Entity:Diagnosis')->findNonDeleted($id);
		
		// Asserts conditions
		$app->assertion->entityExists($diagnosis);
		$app->assertion->entityUpdated($diagnosis, $version);
		
		// Edits the diagnosis
		$diagnosis->setLastEditionDateTime();
		$diagnosis->setName($name);
		$diagnosis->setLastEditor($user);
		$app->data->merge($diagnosis);
	}
