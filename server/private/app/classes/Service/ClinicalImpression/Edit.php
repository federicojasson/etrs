...
/**
 * Represents the /clinical-impression/edit service.
 */
class Edit extends \App\Service\External {
	...
	protected function execute() {
		...
		$clinicalImpression = $app->data->getRepository('Entity:ClinicalImpression')->findNonDeleted($id);
		
		// Asserts conditions
		$app->assertion->entityExists($clinicalImpression);
		$app->assertion->entityUpdated($clinicalImpression, $version);
		
		// Edits the clinical impression
		$clinicalImpression->setLastEditionDateTime();
		$clinicalImpression->setName($name);
		$clinicalImpression->setLastEditor($user);
		$app->data->merge($clinicalImpression);
	}
