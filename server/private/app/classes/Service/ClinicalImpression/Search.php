...
namespace App\Service\ClinicalImpression;

/**
 * Represents the /clinical-impression/search service.
 */
class Search extends \App\Service\External {
	...
	protected function execute() {
		...
		$queryBuilder = $app->data->createQueryBuilder()
			->select('ci.id')
			->from('Entity:ClinicalImpression', 'ci')
			->where('ci.deleted = false');
		
		if (! is_null($expression)) {
			// A full-text search is to be performed
			$queryBuilder
				->andWhere('MATCH(ci.name) AGAINST(:expression BOOLEAN) > 0')
				->setParameter('expression', $expression);
		}
		
		// Sets the sorting criteria
		foreach ($sortingCriteria as $sortingCriterion) {
			$queryBuilder->addOrderBy('ci.' . $sortingCriterion['field'], $sortingCriterion['direction']);
		}
		
		// Searches the clinical impressions
		$clinicalImpressions = $queryBuilder
			->getQuery()
		...
		// Sets an output
		$this->setOutputValue('results', $clinicalImpressions, createArrayFilter(function($clinicalImpression) {
			return bin2hex($clinicalImpression['id']);
		}));
