...
/**
 * Represents the /imaging-test/search service.
 */
class Search extends \App\Service\External {
	...
	protected function execute() {
		...
		if (! is_null($expression)) {
			...
			$queryBuilder
				->andWhere('MATCH(it.name) AGAINST(:expression BOOLEAN) > 0')
				->setParameter('expression', $expression);
		}
		
		// Sets the sorting criteria
		foreach ($sortingCriteria as $sortingCriterion) {
			$queryBuilder->addOrderBy('it.' . $sortingCriterion['field'], $sortingCriterion['direction']);
		}
		
		// Searches the imaging tests
		$imagingTests = $queryBuilder
			->getQuery()
		...
		// Sets an output
		$this->setOutputValue('results', $imagingTests, createArrayFilter(function($imagingTest) {
