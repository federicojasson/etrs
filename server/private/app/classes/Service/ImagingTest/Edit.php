...
/**
 * Represents the /imaging-test/edit service.
 */
class Edit extends \App\Service\External {
	...
	protected function execute() {
		...
		$imagingTest = $app->data->getRepository('Entity:ImagingTest')->findNonDeleted($id);
		
		// Asserts conditions
		$app->assertion->entityExists($imagingTest);
		$app->assertion->entityUpdated($imagingTest, $version);
		
		// Edits the imaging test
		$imagingTest->setLastEditionDateTime();
		$imagingTest->setDataTypeDefinition($dataTypeDefinition);
		$imagingTest->setName($name);
		$imagingTest->setLastEditor($user);
		$app->data->merge($imagingTest);
	}
