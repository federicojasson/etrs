...
/**
 * Represents the /cognitive-test/search service.
 */
class Search extends \App\Service\External {
	...
	protected function execute() {
		...
		if (! is_null($expression)) {
			...
			$queryBuilder
				->andWhere('MATCH(ct.name) AGAINST(:expression BOOLEAN) > 0')
				->setParameter('expression', $expression);
		}
		
		// Sets the sorting criteria
		foreach ($sortingCriteria as $sortingCriterion) {
			$queryBuilder->addOrderBy('ct.' . $sortingCriterion['field'], $sortingCriterion['direction']);
		}
