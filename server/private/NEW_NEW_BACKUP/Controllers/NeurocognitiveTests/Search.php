...
class Search extends \App\Controllers\SecureController {
	...
	protected function call() {
		...
		if (is_null($expression)) {
			...
			$neurocognitiveTests = $app->businessLogicDatabase->searchAllNonDeletedNeurocognitiveTests($sorting, $limit, $offset);
		} else {
			// Only specific neurocognitive tests should be included in the
			// search
			$neurocognitiveTests = $app->businessLogicDatabase->searchSpecificNonDeletedNeurocognitiveTests($sorting, $expression, $limit, $offset);
		}
