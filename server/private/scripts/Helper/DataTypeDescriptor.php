 ...
 */
class DataTypeDescriptor extends Helper {
	 ...
	 */
	public function create($formattedDescriptor) {
		...
		// Gets the definition
		$definition = $this->getDefinition($fields);
	 ...
	 * Throws an exception if the operation fails.
	 */
	private function getDefinition($fields) {
		...
		switch ($type) {
			case DATA_TYPE_BOOLEAN: {
				$definition = $this->processBooleanDefinition($definition);
				break;
			}
			
			case DATA_TYPE_INTEGER_FIX_VALUES: {
				$definition = $this->processIntegerFixValuesDefinition($definition);
				break;
			}
			
			case DATA_TYPE_INTEGER_RANGE: {
				$definition = $this->processIntegerRangeDefinition($definition);
				break;
			}
		}
	 ...
	 * Throws an exception if the operation fails.
	 */
	private function processBooleanDefinition($definition) {
	 ...
	 * Throws an exception if the operation fails.
	 */
	private function processIntegerFixValuesDefinition($definition) {
	 ...
	 * Throws an exception if the operation fails.
	 */
	private function processIntegerRangeDefinition($definition) {
		if (count($definition) !== 2) {
			// The definition doesn't contain exactly 2 entries
			throw new Exception();
		}
