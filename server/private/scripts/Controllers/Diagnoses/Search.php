...
class Search extends \App\Controllers\SecureController {
	...
	protected function call() {
		...
		if (is_null($expression)) {
			// All diagnoses should be included in the search
			
			// Searches the diagnoses
			$diagnoses = $app->businessLogicDatabase->searchAllNonErasedDiagnoses($orderByClause, $limit, $offset);
		} else {
			// Only specific diagnoses should be included in the search
			
			// Gets a boolean expression
			$booleanExpression = getBooleanExpression($expression);
			
			// Searches the diagnoses
			$diagnoses = $app->businessLogicDatabase->searchSpecificNonErasedDiagnoses($booleanExpression, $orderByClause, $limit, $offset);
		}
		...
		for ($i = 0; $i < $count; $i++) {
			$results[$i] = bin2hex($diagnoses[$i]['id']);
		}
