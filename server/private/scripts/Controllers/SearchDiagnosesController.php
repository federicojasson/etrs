 ...
 *	Method:	POST
 */
class SearchDiagnosesController extends SecureController {
	...
	protected function execute() {
		...
		if ($query === '*') {
			...
			$diagnoses = $businessLogicDatabase->selectNonErasedDiagnoses();
		} else {
			...
			// Selects the diagnoses
			$diagnoses = $businessLogicDatabase->selectNonErasedDiagnosesByFullTextSearch($searchExpression, $limit, $offset); // TODO: implement
		}
		
		// Filters the diagnoses and gets their IDs
		$filteredDiagnosisIds = [];
		$count = count($diagnoses);
		for ($i = 0; $i < $count; $i++) {
			$filteredDiagnosisIds[$i] = $app->dataFilter->filterDiagnosis($diagnoses[$i])['id'];
		}
