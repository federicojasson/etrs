 ...
 *	Method:	POST
 */
class SearchNeurocognitiveTestsController extends SecureController {
	...
	protected function execute() {
		...
		if ($query === '*') {
			...
			$neurocognitiveTests = $businessLogicDatabase->selectNonErasedNeurocognitiveTests();
		} else {
			...
			// Selects the clinical impressions
			$neurocognitiveTests = $businessLogicDatabase->selectNonErasedNeurocognitiveTestsByFullTextSearch($searchExpression, $limit, $offset); // TODO: implement
		}
		
		// Filters the clinical impressions and gets their IDs
		$filteredNeurocognitiveTestIds = [];
		$count = count($neurocognitiveTests);
		for ($i = 0; $i < $count; $i++) {
			$filteredNeurocognitiveTestIds[$i] = $app->dataFilter->filterNeurocognitiveTest($neurocognitiveTests[$i])['id'];
		}
		...
		$app->response->setBody([
			...
			'results' => $filteredNeurocognitiveTestIds
		]);
	}
