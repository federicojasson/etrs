...
class Search extends \App\Controllers\SecureController {
	...
	protected function call() {
		...
		if (is_null($expression)) {
			...
			$treatments = $app->businessLogicDatabase->searchAllNonErasedTreatments($orderByClause, $limit, $offset);
		} else {
			...
			// Searches the treatments
			$treatments = $app->businessLogicDatabase->searchSpecificNonErasedTreatments($booleanExpression, $orderByClause, $limit, $offset);
		}
		...
		for ($i = 0; $i < $count; $i++) {
			$results[$i] = bin2hex($treatments[$i]['id']);
		}
