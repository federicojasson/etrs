 ...
 *	Method:	POST
 */
class SearchLaboratoryTestsController extends SecureController {
	...
	protected function execute() {
		...
		if ($query === '*') {
			...
			$laboratoryTests = $businessLogicDatabase->selectNonErasedLaboratoryTests();
		} else {
			...
			// Selects the clinical impressions
			$laboratoryTests = $businessLogicDatabase->selectNonErasedLaboratoryTestsByFullTextSearch($searchExpression, $limit, $offset); // TODO: implement
		}
		
		// Filters the clinical impressions and gets their IDs
		$filteredLaboratoryTestIds = [];
		$count = count($laboratoryTests);
		for ($i = 0; $i < $count; $i++) {
			$filteredLaboratoryTestIds[$i] = $app->dataFilter->filterLaboratoryTest($laboratoryTests[$i])['id'];
		}
		...
		$app->response->setBody([
			...
			'results' => $filteredLaboratoryTestIds
		]);
	}
