...
class Search extends \App\Controllers\SecureController {
	...
	protected function call() {
		...
		if (is_null($expression)) {
			...
			$laboratoryTests = $app->businessLogicDatabase->searchAllNonErasedLaboratoryTests($orderByClause, $limit, $offset);
		} else {
			...
			// Searches the laboratory tests
			$laboratoryTests = $app->businessLogicDatabase->searchSpecificNonErasedLaboratoryTests($booleanExpression, $orderByClause, $limit, $offset);
		}
		...
		for ($i = 0; $i < $count; $i++) {
			$results[$i] = bin2hex($laboratoryTests[$i]['id']);
		}
