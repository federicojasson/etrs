 ...
 *	Method:	POST
 */
class SearchMedicationsController extends SecureController {
	...
	protected function execute() {
		...
		if ($query === '*') {
			...
			$medications = $businessLogicDatabase->selectNonErasedMedications();
		} else {
			...
			// Selects the medications
			$medications = $businessLogicDatabase->selectNonErasedMedicationsByFullTextSearch($searchExpression, $limit, $offset); // TODO: implement
		}
		
		// Filters the medications and gets their IDs
		$filteredMedicationIds = [];
		$count = count($medications);
		for ($i = 0; $i < $count; $i++) {
			$filteredMedicationIds[$i] = $app->dataFilter->filterMedication($medications[$i])['id'];
		}
