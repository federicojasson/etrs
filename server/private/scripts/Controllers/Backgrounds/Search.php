...
class Search extends \App\Controllers\SecureController {
	...
	protected function call() {
		...
		if (is_null($expression)) {
			// All backgrounds should be included in the search
			
			// Searches the backgrounds
			$backgrounds = $app->businessLogicDatabase->searchAllNonErasedBackgrounds($orderByClause, $limit, $offset);
		} else {
			// Only specific backgrounds should be included in the search
			
			// Gets a boolean expression
			$booleanExpression = getBooleanExpression($expression);
			
			// Searches the backgrounds
			$backgrounds = $app->businessLogicDatabase->searchSpecificNonErasedBackgrounds($booleanExpression, $orderByClause, $limit, $offset);
		}
		...
		for ($i = 0; $i < $count; $i++) {
			$results[$i] = bin2hex($backgrounds[$i]['id']);
		}
