...
(function() {
	...
	function TreatmentsViewController($scope, DeleteTreatmentAction, SearchTreatmentsAction, data, SearchHandler, utility) {
		...
		_this.getTemplateUrl = function() {
			return 'app/view/treatments/treatments.html';
		};
		...
		/**
		 * Initializes the search-treatments action.
		 */
		function initializeSearchTreatmentsAction() {
			// Initializes the action
			var action = new SearchTreatmentsAction();
			...
			action.successCallback = function(results, total) {
				...
				// Gets the treatments
				data.getTreatmentArray(results).then(function(treatments) {
					// Refreshes the treatments
			...
			// Includes the action
			$scope.searchTreatmentsAction = action;
