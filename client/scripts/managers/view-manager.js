...
(function() {
	 ...
	 *   user role.
	 * 
	 * The special value '__' can be used to refer to not logged in users when
	 * defining the rules.
	 * 
	 * Be aware that the property templateUrl takes precedence over templateUrls
	 * when deciding which view to load.
	 */
	function viewManagerService($location, $rootScope, $route, authenticationManager, builder, errorManager) {
		var service = this;
		...
		function checkUserAuthorization() {
			...
			if (angular.isUndefined(authorizedUserRoles)) {
			...
			if (! authenticationManager.isUserLoggedIn()) {
				...
				var isAuthorized = authorizedUserRoles.indexOf('__') > -1;
		...
		$rootScope.$on('$routeChangeError', function() {
			...
			var error = builder.buildFatalError('No fue posible cargar recursos esenciales para la aplicación.');
